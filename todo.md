...
- [ ] Build pipeline script mirroring fea-lite/cutlist-optimizer's (`--target web` glue → web repo `public/apps/bug-report-builder/`)
...
- [ ] `pro` gates: multi-screenshot sequences, auto-generated Markdown/PDF
      report assembly, scrolling capture, global hotkeys for capture
- [ ] Pro UI: capture sequence timeline, report preview, "Export
      Markdown/PDF", hotkey settings
- [ ] Desktop shell via `tpt-appfront-webview`; Windows exe build + smoke test
...
- [ ] Gumroad product from apps.md listing copy ($15), exe upload, URL into
      registry — manual step, listing copy at apps.md §T2-8

## Phase 4 — Ship & measure
- [ ] Registry entry (`developer` category, `wasm: { entry: '/apps/bug-report-builder/bug-report-builder.js' }`)
      per the "Adding a finished app to the hub" runbook at the bottom of apps.md
- [ ] Verify hub card/detail/sitemap/runner in dev, then prod after deploy
- [ ] WASM size budget check on the shipped build
